...
export function WizardPageShell({ assessmentType }: WizardPageShellProps) {
  ...
  const scoredQuestions = wizard.questions.filter((question) => question.weight > 0).length;
  const profileQuestions = wizard.questions.filter((question) => question.weight === 0).length;
      ...
      <section className="container pt-8">
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="p-6 text-sm leading-7 text-muted-foreground">
            {wizard.disclaimer} Le questionnaire contient {wizard.questions.length} questions au total: {profileQuestions} questions
            de profil et {scoredQuestions} questions scorées.
          </CardContent>
        </Card>
      </section>
      <section className="container pt-6">
        <Card>
          <CardContent className="space-y-4 p-6 text-sm leading-7 text-muted-foreground">
            <p>
              Besoin d'un rappel avant de répondre ou d'un échange avec notre équipe sur votre contexte? Vous pouvez
              consulter la page du service ou nous écrire directement.
            </p>
            <div className="flex flex-col gap-3 sm:flex-row">
              <Button asChild variant="secondary">
                <Link href={diagnostic.path}>Voir le service {diagnostic.label}</Link>
              </Button>
              <Button asChild variant="ghost">
                <Link href={`/contact?source=${diagnostic.leadSource}-wizard`}>Parler à ForméducWeb</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
