# Etape 10 - Contenu minimum avant promotion

## Dossier
Ce dossier regroupe les contenus minimum prets a publier avant d'amplifier la promotion.

## Fichiers
- `article-01-loi25-cest-quoi-simplement.md`
- `article-02-resume-loi25-pour-pme.md`
- `article-03-loi25-et-site-web-quoi-verifier.md`
- `faq-loi25-10-questions.md`
- `mini-publications-linkedin.md`
- `mini-publications-facebook.md`
- `extraits-courriel-reutilisables.md`

## Regles de ton
- Clair
- Concret
- Non alarmiste
- Sans promesse de conformite garantie
- Sans avis juridique
